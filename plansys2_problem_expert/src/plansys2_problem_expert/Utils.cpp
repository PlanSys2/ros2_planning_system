    ...
    case plansys2_msgs::msg::Node::AND: {
      ...
      for (auto & child_id : tree.nodes[node_id].children) {
        std::tuple<bool, bool, double> result =
          evaluate(tree, problem_client, predicates, functions, apply, use_state, child_id, negate);
    ...
    case plansys2_msgs::msg::Node::FUNCTION: {
      ...
      if (use_state) {
        auto it = std::find_if(
          ...
          std::bind(&parser::pddl::checkNodeEquality, std::placeholders::_1, tree.nodes[node_id]));
        if (it != functions.end()) {
          value = it->value;
        } else {
          success = false;
...
std::tuple<bool, bool, double> evaluate(
  ...
  std::vector<plansys2::Function> & functions, bool apply, uint32_t node_id)
{
  std::shared_ptr<plansys2::ProblemExpertClient> problem_client;
...
bool check(
  const plansys2_msgs::msg::Tree & tree, std::vector<plansys2::Predicate> & predicates,
  std::vector<plansys2::Function> & functions, uint32_t node_id)
{
