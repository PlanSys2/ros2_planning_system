...
ProblemExpertNode::ProblemExpertNode() : rclcpp_lifecycle::LifecycleNode("problem_expert")
{
  ...
  add_problem_conditional_service_ = create_service<plansys2_msgs::srv::AffectConditional>(
    ...
    std::bind(
      &ProblemExpertNode::add_problem_conditional_service_callback, this, std::placeholders::_1,
      std::placeholders::_2, std::placeholders::_3));
  ...
  get_problem_conditionals_service_ = create_service<plansys2_msgs::srv::GetConditionals>(
    ...
    std::bind(
      &ProblemExpertNode::get_problem_conditionals_service_callback, this, std::placeholders::_1,
      std::placeholders::_2, std::placeholders::_3));
  ...
  remove_problem_conditional_service_ = create_service<plansys2_msgs::srv::AffectConditional>(
    ...
    std::bind(
      &ProblemExpertNode::remove_problem_conditional_service_callback, this, std::placeholders::_1,
      std::placeholders::_2, std::placeholders::_3));
  ...
  exist_problem_conditional_service_ = create_service<plansys2_msgs::srv::ExistConditional>(
    ...
    std::bind(
      &ProblemExpertNode::exist_problem_conditional_service_callback, this, std::placeholders::_1,
      std::placeholders::_2, std::placeholders::_3));
...
CallbackReturnT ProblemExpertNode::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  // (fmrico) Here we could have a discussion if we should read the domain from
  // file or from the domain_expert service. Then, we should configure first
  // domain_expert node
...
void ProblemExpertNode::add_problem_conditional_service_callback(
  const std::shared_ptr<rmw_request_id_t> request_header,
  ...
  const std::shared_ptr<plansys2_msgs::srv::AffectConditional::Response> response)
{
  if (problem_expert_ == nullptr) {
    ...
    response->success = problem_expert_->addConditional(request->tree);
    if (response->success) {
      update_pub_->publish(std_msgs::msg::Empty());
      knowledge_pub_->publish(*get_knowledge_as_msg());
    } else {
      response->error_info = "Condition [" + parser::pddl::toString(request->tree) + "] not valid";
    }
  }
}
...
void ProblemExpertNode::get_problem_conditionals_service_callback(
  const std::shared_ptr<rmw_request_id_t> request_header,
  ...
  const std::shared_ptr<plansys2_msgs::srv::GetConditionals::Response> response)
{
  if (problem_expert_ == nullptr) {
    ...
    response->success = true;
    response->conditionals = problem_expert_->getConditionals();
  }
}
...
void ProblemExpertNode::remove_problem_conditional_service_callback(
  const std::shared_ptr<rmw_request_id_t> request_header,
  const std::shared_ptr<plansys2_msgs::srv::AffectConditional::Request> request,
  const std::shared_ptr<plansys2_msgs::srv::AffectConditional::Response> response)
{
  if (problem_expert_ == nullptr) {
    ...
    response->success = problem_expert_->removeConditional(request->tree);
    if (response->success) {
      update_pub_->publish(std_msgs::msg::Empty());
      knowledge_pub_->publish(*get_knowledge_as_msg());
    } else {
...
void ProblemExpertNode::exist_problem_conditional_service_callback(
  const std::shared_ptr<rmw_request_id_t> request_header,
...
plansys2_msgs::msg::Knowledge::SharedPtr ProblemExpertNode::get_knowledge_as_msg() const
{
  ...
  for (const auto & cond : problem_expert_->getConditionals()) {
    ret_msgs->conditionals.push_back(parser::pddl::toString(cond));
  }
