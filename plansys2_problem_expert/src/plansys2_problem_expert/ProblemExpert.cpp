...
bool ProblemExpert::addConditional(const plansys2_msgs::msg::Tree & condition)
{
  if (!existConditional(condition)) {
    if (isValidCondition(condition)) {
      if (
        condition.nodes[0].node_type == plansys2_msgs::msg::Node::ONE_OF &&
...
bool ProblemExpert::removeConditional(const plansys2_msgs::msg::Tree & condition)
{
  ...
  auto it = std::find_if(
    conditionals_.begin(), conditionals_.end(),
    [&condition](const auto & ele) { return parser::pddl::checkTreeEquality(ele, condition); });
...
bool ProblemExpert::existConditional(const plansys2_msgs::msg::Tree & condition)
{
  ...
  switch (tree.nodes[node_id].node_type) {
    ...
    case plansys2_msgs::msg::Node::UNKNOWN: {
      return tree.nodes[node_id].children.size() == 1 &&
             checkPredicateTreeTypes(tree, domain_expert, tree.nodes[node_id].children[0]);
    }
    ...
    default:
      // LCOV_EXCL_START
      std::cerr << "checkPredicateTreeTypes: Error parsing expression ["
  ...
  for (auto cond : conditionals_) {
    ...
    switch (root_node.node_type) {
      case plansys2_msgs::msg::Node::UNKNOWN: {
        ...
        std::transform(
          predicate.name.begin(), predicate.name.end(), predicate.name.begin(), ::tolower);
...
bool ProblemExpert::addProblem(const std::string & problem_str)
{
  ...
  for (auto cond : problem.init_cond) {
    ...
    switch (tree_node->node_type) {
      ...
      case plansys2_msgs::msg::Node::ONE_OF: {
        auto tmp(*tree_node);
        std::cout << "Adding oneof: " << parser::pddl::toString(tree, tree_node->node_id)
                  << std::endl;
        if (!addConditional(tree)) {
          std::cerr << "Failed to add oneof : " << parser::pddl::toString(tree, tree_node->node_id)
                    << std::endl;
        }
        break;
      }
      case plansys2_msgs::msg::Node::OR: {
        ...
        std::cout << "Adding or node: " << parser::pddl::toString(or_tree, or_node->node_id)
                  << std::endl;
        if (!addConditional(tree)) {
          std::cerr << "Failed to add or : " << parser::pddl::toString(or_tree, or_node->node_id)
                    << std::endl;
        }
        break;
      }
  ...
  for (auto ground : problem.goal) {
    ...
    std::cout << "Adding goal node: " << parser::pddl::toString(goal, goal_node->node_id)
              << std::endl;
