...
namespace pddl
{
Ground::Ground(const Ground * g, Domain & d) : ParamCond(g), lifted(d.preds.get(g->name)) {}
  ...
  std::ostream & s, unsigned indent, const TokenStruct<std::string> & ts, const Domain & d) const
{
  ...
  for (unsigned i = 0; i < params.size(); ++i) {
    if (ts.size() && params[i] >= 0 && (unsigned)params[i] < ts.size())
      s << " " << ts[params[i]];
    else if (params[i] >= 0 && (unsigned)params[i] >= ts.size())
      s << " ?" << params[i];
    else
  ...
  plansys2_msgs::msg::Tree & tree, const Domain & d, const std::vector<std::string> & replace) const
{
  ...
  for (unsigned i = 0; i < params.size(); ++i) {
    ...
    if (i < replace.size()) {
      ...
      } else  // param has a constant value; retrieve from
