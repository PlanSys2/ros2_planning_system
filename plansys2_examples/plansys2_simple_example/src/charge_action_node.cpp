...
class ChargeAction : public rclcpp::Node
{
public:
  ...
  ChargeAction()
  : rclcpp::Node("charge"), canceled_(false)
  {
    using namespace std::placeholders;

    this->execute_Charge_action_server_ = rclcpp_action::create_server<ExecuteAction>(
      this->get_node_base_interface(),
      this->get_node_clock_interface(),
      this->get_node_logging_interface(),
      this->get_node_waitables_interface(),
      "charge",
      std::bind(&ChargeAction::handle_goal, this, _1, _2),
      std::bind(&ChargeAction::handle_cancel, this, _1),
      std::bind(&ChargeAction::handle_accepted, this, _1));
  }

private:
  rclcpp_action::Server<ExecuteAction>::SharedPtr execute_Charge_action_server_;
  bool canceled_;
  ...
  {
    RCLCPP_INFO(this->get_logger(), "Received request to cancel Charge action");
  ...
  {
    using namespace std::placeholders;
    std::thread{std::bind(&ChargeAction::execute, this, _1), goal_handle}.detach();
  }
  ...
  void execute(const std::shared_ptr<GoalHandleExecuteAction> goal_handle)
  {
    ...
    while (rclcpp::ok() && !canceled_ && feedback->progress < 100) {
      ...
      std::cout << "Charging ... [" << feedback->progress << "%]  " << std::flush;
