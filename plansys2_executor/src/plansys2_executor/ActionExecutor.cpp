...
: status_(STARTING), finished_(false) 
{
  ...
  node_ = std::make_shared<rclcpp::Node>(current_action_.name + "_action_client");
  domain_client_ = std::make_shared<plansys2::DomainExpertClient>(node_);
  problem_client_ = std::make_shared<plansys2::ProblemExpertClient>(node_);
  ...
  if (check(current_action_.at_start_requirements)) {
    ...
    std::cerr << "At start correct " << std::endl;
    ...
    std::cerr << "At start failed " << std::endl;
    status_ = AT_START_ERROR;
    finished_ = true;
  }
}
...
void
ActionExecutor::feedback_callback(GoalHandleExecuteAction::SharedPtr,
...
ActionExecutor::result_callback(const GoalHandleExecuteAction::WrappedResult & result)
{
  ...
  if (result.result->success) {
    RCLCPP_INFO(node_->get_logger(), "Result action received: Success");
  } else {
    RCLCPP_INFO(node_->get_logger(), "Result action received: Fail [%s]",
...
ActionExecutor::update_current_action(const std::string & action_expr)
{
  ...
  } else if (durative_action.has_value()) 
  {
    ...
    RCLCPP_ERROR(node_->get_logger(), "Action [%s] not found", current_action_.name.c_str());
    return false;
  }
}
