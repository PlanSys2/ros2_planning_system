...
ExecutorNodeContingent::ExecutorNodeContingent() : ExecutorNodeBase()
{
  using namespace std::placeholders;
}

rclcpp_action::GoalResponse ExecutorNodeContingent::handle_goal(
...
void ExecutorNodeContingent::execute(const std::shared_ptr<GoalHandleExecutePlan> goal_handle)
{
  ...
  while (status == BT::NodeStatus::RUNNING && !cancel_plan_requested_) {
    ...
    //      dotgraph_msg.data = bt_builder->get_dotgraph(
    //          action_map,
    //          this->get_parameter("enable_dotgraph_legend").as_bool());
    //      dotgraph_pub_->publish(dotgraph_msg);
