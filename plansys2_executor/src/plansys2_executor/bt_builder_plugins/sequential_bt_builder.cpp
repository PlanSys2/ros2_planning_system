...
std::string SequentialBTBuilder::add_action_to_bt(
  const plansys2_msgs::msg::PlanItem & plan_item, const std::vector<plansys2_msgs::msg::PlanItem> & previous_items)
{
  std::string ret;
  std::string ret_aux = bt_action_;
  
  const std::string action_id = plan_item.action +
    ":" + std::to_string(static_cast<int>(plan_item.time * 1000));

  std::string wait_actions;
  for (const auto & previous_item : previous_items) {
    const std::string parent_action_id =
      previous_item.action +
      ":" + std::to_string(static_cast<int>(previous_item.time * 1000));
    wait_actions = wait_actions + add_tabs(1) + "<WaitAction action=\"" + parent_action_id + "\"/>\n";
  }
...
std::string SequentialBTBuilder::get_tree(const plansys2_msgs::msg::Plan & current_plan)
{
  ...
  bt_string = std::string("<root BTCPP_format=\"4\" main_tree_to_execute=\"MainTree\">\n") + add_tabs(1) +
      "<BehaviorTree ID=\"MainTree\">\n" + add_tabs(2) + "<Sequence name=\"MainSequence\">\n";
