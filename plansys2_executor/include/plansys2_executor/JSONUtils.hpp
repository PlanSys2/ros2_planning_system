...
#ifndef PLANSYS2_EXECUTOR__JSONUTILS_HPP_
#define PLANSYS2_EXECUTOR__JSONUTILS_HPP_
  ...
  std::unordered_map<std::string, std::string> & dest)
{
  if (js.is_object()) {
    ...
    for (auto & [key, value] : js.items()) {
      if (value.is_string()) {
        dest[key] = value.get<std::string>();
      } else {
        throw std::runtime_error(
            "Invalid JSON for std::unordered_map<std::string, std::string>: value must be string");
      }
    }
  } else {
    throw std::runtime_error(
        "Invalid JSON for std::unordered_map<std::string, std::string>: must be object");
  }
}

inline void to_json(nlohmann::json & js, const std::unordered_map<std::string, std::string> & src)
{
...
#endif  // PLANSYS2_EXECUTOR__JSONUTILS_HPP_
