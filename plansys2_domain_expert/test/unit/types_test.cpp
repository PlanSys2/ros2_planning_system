...
TEST(domain_types, predicate_tree_to_string_2)
{
  ...
  std::shared_ptr<plansys2::NotNode> pn_not_2 = std::make_shared<plansys2::NotNode>();
  pn_not_2->op = pn_not;
...
TEST(domain_types, predicate_tree_to_string_3)
{
  ...
  std::shared_ptr<plansys2::NotNode> pn_not_3 = std::make_shared<plansys2::NotNode>();
  pn_not_3->op = pn_or;
...
TEST(domain_types, predicate_tree_from_string)
{
  ...
  std::shared_ptr<plansys2::AndNode> and_node =
    std::dynamic_pointer_cast<plansys2::AndNode>(tree.root_);
  ASSERT_EQ(and_node->type_, plansys2::AND);
  ASSERT_FALSE(and_node->negate_);

  std::shared_ptr<plansys2::PredicateNode> p1_node =
    std::dynamic_pointer_cast<plansys2::PredicateNode>(and_node->ops[0]);
  ASSERT_EQ(p1_node->type_, plansys2::PREDICATE);
  ...
  std::shared_ptr<plansys2::NotNode> pnot_node =
    std::dynamic_pointer_cast<plansys2::NotNode>(and_node->ops[1]);
  ASSERT_EQ(pnot_node->type_, plansys2::NOT);
  ASSERT_FALSE(pnot_node->negate_);

  std::shared_ptr<plansys2::PredicateNode> p2_node =
    std::dynamic_pointer_cast<plansys2::PredicateNode>(pnot_node->op);
  ASSERT_EQ(p2_node->type_, plansys2::PREDICATE);
...
TEST(domain_types, predicate_tree_from_string_2)
{
  ...
  ASSERT_NE(tree.root_, nullptr);
  std::shared_ptr<plansys2::NotNode> not_node =
    std::dynamic_pointer_cast<plansys2::NotNode>(tree.root_);
  ASSERT_NE(not_node, nullptr);
  ASSERT_EQ(not_node->type_, plansys2::NOT);
  ASSERT_FALSE(not_node->negate_);

  std::shared_ptr<plansys2::AndNode> and_node =
    std::dynamic_pointer_cast<plansys2::AndNode>(not_node->op);
  ASSERT_EQ(and_node->type_, plansys2::AND);
  ...
  std::shared_ptr<plansys2::PredicateNode> p2_node =
    std::dynamic_pointer_cast<plansys2::PredicateNode>(and_node->ops[1]);
  ASSERT_EQ(p2_node->type_, plansys2::PREDICATE);
  ASSERT_EQ(p2_node->predicate_.name, "robot_at");
  ASSERT_EQ(p2_node->predicate_.parameters[0].name, "r2d2");
  ASSERT_EQ(p2_node->predicate_.parameters[1].name, "kitchen");
...
TEST(domain_types, get_predicates)
{
  ...
  plansys2::PredicateTree tree_1;
  tree_1.fromString(expresion_1);

  std::vector<plansys2::Predicate> predicates_1;
