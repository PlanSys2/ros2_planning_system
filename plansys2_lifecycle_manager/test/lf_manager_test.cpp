...
TEST(lifecycle_manager, lf_startup)
{
  ...
  manager_nodes["problem_expert"] =
    std::make_shared<plansys2::LifecycleServiceClient>("domain_expert_lc_mngr", "problem_expert");
  manager_nodes["planner"] =
    std::make_shared<plansys2::LifecycleServiceClient>("domain_expert_lc_mngr", "planner");
  manager_nodes["executor"] =
    std::make_shared<plansys2::LifecycleServiceClient>("domain_expert_lc_mngr", "executor");
  ...
  ASSERT_EQ(de_node->get_current_state().id(), lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE);

  ASSERT_EQ(pe_node->get_current_state().id(), lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE);

  ASSERT_EQ(pl_node->get_current_state().id(), lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE);

  ASSERT_EQ(ex_node->get_current_state().id(), lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE);
