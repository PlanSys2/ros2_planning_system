  ...
  default_types_{}
{
  get_plan_service_ = create_service<plansys2_msgs::srv::GetPlan>(
    "planner/get_plan", std::bind(
                          &PlannerNode::get_plan_service_callback, this, std::placeholders::_1,
...
CallbackReturnT PlannerNode::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  if (!solver_ids_.empty()) {
    ...
    for (size_t i = 0; i != solver_types_.size(); i++) {
      try {
        ...
        RCLCPP_INFO(
          get_logger(), "Created solver : %s of type %s", solver_ids_[i].c_str(),
          solver_types_[i].c_str());
    ...
    RCLCPP_INFO(
      get_logger(), "Created default solver : %s of type %s", "POPF", "plansys2/POPFPlanSolver");
  }
...
CallbackReturnT PlannerNode::on_activate(const rclcpp_lifecycle::State & state)
{
...
CallbackReturnT PlannerNode::on_deactivate(const rclcpp_lifecycle::State & state)
{
...
CallbackReturnT PlannerNode::on_cleanup(const rclcpp_lifecycle::State & state)
{
...
CallbackReturnT PlannerNode::on_shutdown(const rclcpp_lifecycle::State & state)
{
...
CallbackReturnT PlannerNode::on_error(const rclcpp_lifecycle::State & state)
{
...
void PlannerNode::get_plan_service_callback(
  const std::shared_ptr<rmw_request_id_t> request_header,
  const std::shared_ptr<plansys2_msgs::srv::GetPlan::Request> request,
  const std::shared_ptr<plansys2_msgs::srv::GetPlan::Response> response)
{
  auto plan = solvers_.begin()->second->getPlan(request->domain, request->problem, get_namespace());
